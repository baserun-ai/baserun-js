...
describe('anthropic', () => {
  ...
  describe('messages api', () => {
    ...
    const max_tokens = 665;
    const imageInput: Anthropic.ImageBlockParam = {
      type: 'image',
      source: {
        type: 'base64',
    ...
    test('non stream', async () => {
      ...
      expectTypeOf(result).toMatchTypeOf<Anthropic.Message>();
      expect(submitLogSpy.mock.calls.length).toEqual(1);
      const span = submitLogSpy.mock.calls[0][0] as Span;

      expect(span.stream).toEqual(false);
      expect(span.model).toEqual(model);
      expect(span.maxTokens).toEqual(max_tokens);
      expect(span.requestType).toEqual(BaserunType.Chat);
      expect(span.vendor).toEqual(BaserunProvider.Anthropic);
    ...
    test('extra params', async () => {
      ...
      expect(span.topK).toEqual(generationConfig.top_k);
      expect(span.topP).toEqual(generationConfig.top_p);
    ...
    test('create stream', async () => {
      ...
      expect(span.stream).toEqual(true);
      expect(span.model).toEqual(model);
      expect(span.maxTokens).toEqual(max_tokens);
      expect(span.requestType).toEqual(BaserunType.Chat);
      expect(span.vendor).toEqual(BaserunProvider.Anthropic);
    ...
    test('stream', async () => {
      ...
      expectTypeOf(result).toMatchTypeOf<MessageStream>();
    ...
    test('multimodal', async () => {
      ...
      const result = await anthropic.messages.create({
        ...
        messages: [
          {
            role: 'user',
            content: [imageInput, { type: 'text', text: prompt }],
          },
        ],
      });
      expectTypeOf(result).toMatchTypeOf<Anthropic.Message>();
    ...
    test('multiple messages input', async () => {
      ...
      expect(span.promptMessages[0].content).toEqual(messages[0].content);
      expect(span.promptMessages[0].role).toEqual(messages[0].role);
      expect(span.promptMessages[1].content).toEqual(messages[1].content);
      expect(span.promptMessages[1].role).toEqual(messages[1].role);
      expect(span.promptMessages[2].content).toEqual(messages[2].content);
      expect(span.promptMessages[2].role).toEqual(messages[2].role);
      expect(span.completions.length).toEqual(1);
      expect(span.completions[0]).toMatchObject({
        finishReason: 'end_turn',
        name: '',
