import {
  SpyInstance,
  expect,
  vi,
  describe,
  beforeAll,
  beforeEach,
  afterEach,
  test,
  expectTypeOf,
} from 'vitest';
import { baserun } from '../index.js';
import { Baserun } from '../baserun.js';
import { Log, Run, Span } from '../v1/gen/baserun.js';
baserun.init();

import {
  GenerateContentResult,
  GenerateContentStreamResult,
  GoogleGenerativeAI,
} from '@google/generative-ai';
import { BaserunProvider, BaserunType } from '../types.js';

const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY!);

const testName = (name: string, shouldStream: boolean, shouldChat: boolean) =>
  [shouldStream && 'stream', shouldChat && 'chat', name]
    .filter((v) => v)
    .join(' ');

describe('googleGenerativeAI', () => {
  let submitLogSpy: SpyInstance<
    [logOrSpan: Log | Span, run: Run, submitRun?: boolean],
    Promise<void>
  >;

  beforeAll(() => {
    baserun.init();
  });

  beforeEach(() => {
    submitLogSpy = vi.spyOn(Baserun, 'submitLogOrSpan');
  });

  afterEach(() => {
    submitLogSpy.mockRestore();
  });

  const modelName = 'gemini-pro';
  const visionModelName = 'gemini-pro-vision';
  const expectedModelName = 'models/gemini-pro';
  const expectedVisionModelName = 'models/gemini-pro-vision';
  const imageInput = {
    inlineData: {
      mimeType: 'image/png',
      data: '',
    },
  };
  const multiTurnPrompt = [
    { parts: [{ text: 'Hello, I have 2 dogs in my house.' }], role: 'user' },
    {
      parts: [{ text: 'Great to meet you. What would you like to know?' }],
      role: 'model',
    },
    { parts: [{ text: 'How many paws are in my house?' }], role: 'user' },
  ];
  const generationConfig = {
    stopSequences: ['red'],
    maxOutputTokens: 200,
    temperature: 0.9,
    topP: 0.1,
    topK: 16,
  };

  describe.each([
    [false, false],
    [true, false],
    [false, true],
    [true, true],
  ])('googleGenerativeAI', (shouldStream, shouldChat) => {
    test(testName('single text input', shouldStream, shouldChat), async () => {
      const prompt = '123+13=';
      const model = genAI.getGenerativeModel({ model: modelName });
      const chat = model.startChat();

      if (!shouldStream) {
        const result = await (shouldChat
          ? chat.sendMessage(prompt)
          : model.generateContent(prompt));
        expectTypeOf(result).toMatchTypeOf<GenerateContentResult>();
      } else {
        const result = await (shouldChat
          ? chat.sendMessageStream(prompt)
          : model.generateContentStream(prompt));
        expectTypeOf(result).toMatchTypeOf<GenerateContentStreamResult>();
        // consume chunks
        for await (const chunk of result.stream) {
          chunk.text();
        }
      }

      expect(submitLogSpy.mock.calls.length).toEqual(1);
      const span = submitLogSpy.mock.calls[0][0] as Span;

      expect(span.stream).toEqual(shouldStream);
      expect(span.model).toEqual(expectedModelName);
      expect(span.requestType).toEqual(BaserunType.Chat);
      expect(span.vendor).toEqual(BaserunProvider.GoogleGenerativeAI);
      expect(span.promptMessages.length).toEqual(1);
      expect(span.promptMessages[0].content).toEqual(prompt);
      expect(span.promptMessages[0].role).toEqual('user');
      expect(span.completions.length).toEqual(1);
      expect(span.completions[0]).toMatchObject({
        finishReason: 'STOP',
        name: '',
        role: undefined,
        toolCallId: '',
        toolCalls: [],
      });
    });

    test(
      testName('prompt with history', shouldStream, shouldChat),
      async () => {
        const model = genAI.getGenerativeModel({ model: modelName });
        const chat = model.startChat({ history: multiTurnPrompt.slice(0, -1) });

        if (!shouldStream) {
          const result = shouldChat
            ? await chat.sendMessage(
                multiTurnPrompt[multiTurnPrompt.length - 1].parts,
              )
            : await model.generateContent({ contents: multiTurnPrompt });
          expectTypeOf(result).toMatchTypeOf<GenerateContentResult>();
        } else {
          const result = shouldChat
            ? await chat.sendMessageStream(
                multiTurnPrompt[multiTurnPrompt.length - 1].parts,
              )
            : await model.generateContentStream({ contents: multiTurnPrompt });
          expectTypeOf(result).toMatchTypeOf<GenerateContentStreamResult>();
          // consume chunks
          for await (const chunk of result.stream) {
            chunk.text();
          }
        }

        expect(submitLogSpy.mock.calls.length).toEqual(1);
        const span = submitLogSpy.mock.calls[0][0] as Span;

        expect(span.stream).toEqual(shouldStream);
        expect(span.model).toEqual(expectedModelName);
        expect(span.requestType).toEqual(BaserunType.Chat);
        expect(span.vendor).toEqual(BaserunProvider.GoogleGenerativeAI);
        expect(span.promptMessages.length).toEqual(3);
        span.promptMessages.forEach((message, ix) => {
          expect(message.content).toEqual(multiTurnPrompt[ix].parts[0].text);
          expect(message.role).toEqual(multiTurnPrompt[ix].role);
        });
        expect(span.completions.length).toEqual(1);
        expect(span.completions[0]).toMatchObject({
          finishReason: 'STOP',
          name: '',
          role: undefined,
          toolCallId: '',
          toolCalls: [],
        });
      },
    );

    test(
      testName('multiple parts and image input', shouldStream, shouldChat),
      async () => {
        const prompt = 'what does this picture show?';
        const model = genAI.getGenerativeModel({ model: visionModelName });
        const chat = model.startChat();

        if (!shouldStream) {
          const result = await (shouldChat
            ? chat.sendMessage([prompt, imageInput])
            : model.generateContent([prompt, imageInput]));
          expectTypeOf(result).toMatchTypeOf<GenerateContentResult>();
        } else {
          const result = await (shouldChat
            ? chat.sendMessageStream([prompt, imageInput])
            : model.generateContentStream([prompt, imageInput]));
          expectTypeOf(result).toMatchTypeOf<GenerateContentStreamResult>();
          // consume chunks
          for await (const chunk of result.stream) {
            chunk.text();
          }
        }

        expect(submitLogSpy.mock.calls.length).toEqual(1);
        const span = submitLogSpy.mock.calls[0][0] as Span;

        expect(span.stream).toEqual(shouldStream);
        expect(span.model).toEqual(expectedVisionModelName);
        expect(span.requestType).toEqual(BaserunType.Chat);
        expect(span.vendor).toEqual(BaserunProvider.GoogleGenerativeAI);
        expect(span.promptMessages.length).toEqual(1);
        expect(span.promptMessages[0].content).toEqual(prompt);
        expect(span.promptMessages[0].role).toEqual('user');
        expect(span.completions.length).toEqual(1);
        expect(span.completions[0]).toMatchObject({
          finishReason: 'STOP',
          name: '',
          role: undefined,
          toolCallId: '',
          toolCalls: [],
        });
      },
    );

    test(testName('additional config', shouldStream, shouldChat), async () => {
      const prompt = '123+13=';
      const model = genAI.getGenerativeModel({
        model: modelName,
        generationConfig: generationConfig,
      });
      const chat = model.startChat({ generationConfig: generationConfig });

      if (!shouldStream) {
        await (shouldChat
          ? chat.sendMessage(prompt)
          : model.generateContent(prompt));
      } else {
        const result = await (shouldChat
          ? chat.sendMessageStream(prompt)
          : model.generateContentStream(prompt));
        // consume chunks
        for await (const chunk of result.stream) {
          chunk.text();
        }
      }

      expect(submitLogSpy.mock.calls.length).toEqual(1);
      const span = submitLogSpy.mock.calls[0][0] as Span;

      expect(span.topK).toEqual(generationConfig.topK);
      expect(span.topP).toEqual(generationConfig.topP);
      expect(span.maxTokens).toEqual(generationConfig.maxOutputTokens);
      expect(span.temperature).toEqual(generationConfig.temperature);
    });

    test(testName('error response', shouldStream, shouldChat), async () => {
      const prompt = 'tell me something racist'; // prompt that will violate safety settings
      const model = genAI.getGenerativeModel({ model: modelName });
      const chat = model.startChat();

      if (!shouldStream) {
        const result = await (shouldChat
          ? chat.sendMessage(prompt)
          : model.generateContent(prompt));
        expectTypeOf(result).toMatchTypeOf<GenerateContentResult>();
        expect(result.response.promptFeedback).toBeDefined();
        expect(result.response.text).toThrowError();
      } else {
        const result = await (shouldChat
          ? chat.sendMessageStream(prompt)
          : model.generateContentStream(prompt));
        expectTypeOf(result).toMatchTypeOf<GenerateContentStreamResult>();
        // consume chunks
        for await (const chunk of result.stream) {
          expect(chunk.promptFeedback).toBeDefined();
          expect(chunk.text).toThrowError();
        }
      }

      expect(submitLogSpy.mock.calls.length).toEqual(1);
      const span = submitLogSpy.mock.calls[0][0] as Span;

      expect(span.stream).toEqual(shouldStream);
      expect(span.model).toEqual(expectedModelName);
      expect(span.requestType).toEqual(BaserunType.Chat);
      expect(span.vendor).toEqual(BaserunProvider.GoogleGenerativeAI);
      expect(span.promptMessages.length).toEqual(1);
      expect(span.promptMessages[0].content).toEqual(prompt);
      expect(span.promptMessages[0].role).toEqual('user');
      expect(span.errorStacktrace).toBeDefined();
    });
  });
});
